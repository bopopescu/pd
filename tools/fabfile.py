...
env.hosts = ['playdation.net']
env.pth = '/var/www/playdation.net/playdation/test/'
env.user = 'braskin'
env.virtualhost_path = "/vhost/"
env.project_name = 'playdation.net'
env.path = '/var/www/playdation.net/playdation/test/'
# tasks

#config(
#    project = 'playdation.net',
#    fab_hosts = ['playdation.net'],
#    fab_user = 'braskin',
#    fab_project_name = 'playdation.net'
#    
#
#
#
...
def local():
    ...
    config.hosts = ['playdation.net']
    config.path = '/var/www/playdation.net/playdation/test/'
    config.user = 'braskin'
    config.virtualhost_path = "/vhost/"
    config.project_name = 'playdation.net'
# tasks
def test():
...
def setup():
    ...
    run('mkdir -p $(pth); cd $(pth); virtualenv .;')
    run('cd $(pth); mkdir -p releases; mkdir -p shared; mkdir -p packages;')
    deploy()
def deploy():
    """
...
def install_requirements():
    ...
    require('release', provided_by=[deploy, setup])
    run('cd $(path); pip install -E . -r ./releases/$(release)/requirements/base.txt')
    run('cd $(path); pip install -E . -r ./releases/$(release)/requirements/project.txt')
